...
\begin{frame}{論文の目的}
  \begin{itemize}
    \item 平均場理論(TDHF)＋経路積分で核分裂反応を記述する
    \item
  \end{itemize}
  ...
  \begin{columns}[t]
    \begin{column}{.5\textwidth}
      \begin{itemize}
        ...
        \item \serifRoman{2}のポテンシャル障壁を乗り越えて\serifRoman{3}の領域へ時間発展
...
\begin{frame}{\ce{^{8}Be -> ^{4}He + ^{4}He}の計算結果}
  ...
  \begin{itemize}
    \item この時間発展の様子はどのように計算してる？
    \item \ce{^{8}Be}のポテンシャルは最初からHFで計算していた？
  \end{itemize}
\end{frame}
\begin{frame}{まとめ}
  \begin{itemize}
    \item 平均場理論に経路積分を取り入れることでトンネル効果を再現できる
    \item HF方程式を解くことで状態を得られるため、少数核子系での数値計算が可能
  \end{itemize}
  重い核（\ce{^{238}U}など）に適用するためには、計算コスト$\order{e^N}$やメモリの問題
