...
\begin{frame}{エネルギー固有値を求める}
  ...
  \begin{align*}
    ...
    \approx & A\sum_{m=1}^{\infty}f_m e^{iW(mT)} =A\sum_{m=1}^{\infty}e^{im\pi}\qty(e^{iW(T)})^m = \frac{-Ae^{iW(T)}}{1+e^{iW(T)}}
  \end{align*}
  \begin{itemize}
...
\begin{frame}
  \frametitle{1粒子系での例}
  \begin{equation*}
    \varDelta E_n = -i\frac{\omega(E_n^{~0})}{2\pi}e^{-W_2(E_n^{~0})}\quad\text{を得る理由}
  \end{equation*}
...
\begin{frame}{核子多体系に適用する}
  \begin{columns}[t]
    \begin{column}{.5\textwidth}
      \begin{itemize}
        \item 原子核の四重極モーメント$Q$が大きいほど原子核が変形
...
%     \item HF方程式を載せる
%     \item 現在のコンピューターでどのくらい時間がかかるのか：基底状態を求めるのは、Qに対してHFは何日もかかる
%     \item サブバリアー核融合とは
%     \item 双極子モーメントを使わない理由
%     \item スレーター行列式とは
% \end{itemize}
% \end{frame}
