  ...
  \begin{itemize}
    ...
    \item 構成粒子がFermi粒子の場合、全体系の波動関数$\ket{\Psi}_{\text{A}}$は任意の2つの粒子の入れ替えに対して反対称になるという要請から、$N$次置換群を$\mathfrak{S}_{N}$として、
          \begin{equation*}
            ...
            \sgn(\sigma)\ket{\phi_{\sigma(1)}}\!\ket{\phi_{\sigma(2)}}\cdots\ket{\phi_{\sigma(N)}}
          \end{equation*}
          と表される。これは、行列式を用いて表現することができる：
          \begin{equation*}
            \ket{\Psi}_{\text{A}}=\frac{1}{\sqrt{N!}}\det
            \begin{bmatrix}
              \ket{\phi_{1}}^{(0)} & \ket{\phi_2}^{(0)} & \cdots & \ket{\phi_N}^{(0)} \\
...
\begin{frame}
  ...
  Fock演算子は、運動項$\hat{t}(\bm{r})$、Hartree項$\hat{\Gamma}_{\text{H}}(\bm{r})$、Fock項$\hat{\Gamma}_{\text{F}}(\bm{r},\bm{r}')$を用いて
  \begin{equation*}
...
\begin{frame}
  \frametitle{Appendix：Hartree-Fock方程式}
  \small
  運動項$\hat{t}(\bm{r})$、Hartree項$\hat{\Gamma}_{\text{H}}(\bm{r})$、Fock項$\hat{\Gamma}_{\text{F}}(\bm{r},\bm{r}')$は、
