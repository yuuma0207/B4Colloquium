...
\begin{frame}{論文の目的}
  \begin{itemize}
    \item 平均場理論(TDHF)＋経路積分でトンネル効果を記述する
...
\begin{frame}{背景}
  \begin{columns}[t]
    \begin{column}{.5\textwidth}
      \begin{itemize}
        \item トンネル効果による原子核反応が存在する
        \item 1次元模型では、ガモフによる$\alpha$崩壊の理論やWKB近似などがある
        \item 実際の核分裂の計算では精度を良くすることが難しい
        \item 平均場理論の枠組みでこれらの反応を記述したい
      \end{itemize}
    \end{column}
    \begin{column}{.5\textwidth}
...
\begin{frame}{平均場理論}
  \begin{itemize}
    ...
    \item 原子核中では、平均ポテンシャルによって核子の配置が決定
...
\begin{frame}{1粒子系での例}
  \begin{columns}[t]
    \begin{column}{.5\textwidth}
      \begin{itemize}
        \item 単純なHF方程式で得られる基底状態$q_1$は準安定状態
        \item $q_2$までトンネリングした後は$q$が増大する方向へ運動する
      \end{itemize}
    \end{column}
    \begin{column}{.5\textwidth}
    ...
    \end{column}
  \end{columns}
  時刻$t$で$q_1$の状態にいる確率$P(t)$は、$E=E_n^{~0}-i\Gamma_n/2$より
  \begin{equation*}
    P(t) = \abs{\braket{q_1}{\psi(t)}}^2 = \abs{\bra{q_1}e^{-iEt/\hbar}\ket{q_1}}^2 = e^{-\Gamma_nt/\hbar}
  \end{equation*}
\end{frame}
...
\begin{frame}{\ce{^{8}Be -> ^{4}He + ^{4}He}の計算結果}
  ...
  \begin{itemize}
    ...
    \item この時間発展の様子はどのように計算してる？ $Q_1\to Q_3$の時間発展は追うことができない？
  \end{itemize}
\end{frame}
\begin{frame}{まとめ}
  \begin{itemize}
